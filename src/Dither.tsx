...
function DitheredWaves({
  ...
  useEffect(() => {
    ...
    if (resolution.x !== width || resolution.y !== height) resolution.set(width, height);
  }, [gl, size, waveUniforms]);

  const previousColor = useRef([...waveColor]);

  useFrame(({ clock }) => {
    if (!disableAnimation) waveUniforms.time.value = clock.getElapsedTime();
    ...
    if (enableMouseInteraction) waveUniforms.mousePos.value.copy(mouseRef.current);
  });
